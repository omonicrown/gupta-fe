      ...
      <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6  rounded">
        <div className="rounded-t mb-0  md:py-1 p-1 border-0">
          <div className="bg-blue-100  rounded-lg ml-3 m-1 p-2 mb-3">
            <span className=" bg-blue-100  rounded-lg  text-gray-500 text-[12px]"><span className="mr-4 text-red-500 bg-red-200 p-1 px-3 rounded-full text-[15px]">!</span> Create custom short links for easy sharing. Track clicks with analytics, and boost accessibility with QR codes generated by vendors.</span>
          </div>
                  ...
                  (data?.redirect_links?.length >= 1) ?
                    <div className="container flex-col md:flex-row md:justify-start mt-1 pt-1 grid lg:grid-cols-3 grid-cols-2 md:gap-1 gap-3">
                      {(inputEl?.current?.value?.length > 1 ? searchResult : data?.redirect_links).map(
                        (data, index) => (
                          <>
                           <div class="max-w-sm md:mx-0 lg:mx-2 my-2  bg-white rounded-lg border border-gray-200 shadow-md">
                            <span className="flex justify-between lg:hidden md:gap-3 rounded-t-lg border-none bg-[#0071BC] px-1 py-1" >
                                <p class="mb-2  lg:flex font-medium tracking-tight text-white mt-2" style={{ fontSize: '12px' }}>{truncateText(`${data?.name}`)}  </p>
                                <div>
                                  <CopyToClipboard text={`${configs?.baseRedirect}/${(data?.name)}`}
                                    onCopy={() => isCopied()}>
                                    <h3 className="text-white text-[10px] mt-1.5 border border-[#fff] py-0.5 px-1 rounded-full cursor-pointer">Copy link</h3>
                                  </CopyToClipboard>
                                </div>
                              </span>
                              ...
                              <span className="lg:flex hidden lg:justify-between md:gap-3 rounded-t-lg border-none bg-[#0071BC] px-3 py-1" >
                                ...
                                <div>
                                  <CopyToClipboard text={`${configs?.baseRedirect}/${(data?.name)}`}
                                    onCopy={() => isCopied()}>
                                    <h3 className="text-white text-xs mt-1.5 border border-[#fff] py-1 px-3 rounded-full cursor-pointer">Copy link</h3>
                                  </CopyToClipboard>
                                </div>
                              </span>
                              ...
                              <NavLink to={`/link-details/${data?.id}`} className={'cursor-pointer'}>
                                <span className="flex justify-end gap-2 m-2 ">
                                  ...
                                  <p class="tracking-tight font-bold " style={{ color: '#149E49', fontSize: '10px', paddingTop: '1px' }}> {data?.short_url?.visits?.filter(visit => visit?.operating_system !== '0').length ? data?.short_url?.visits?.filter(visit => visit?.operating_system !== '0').length : '0'} click(s)</p>
                                </span>
                                <div className="bg-[#F4FBFF] mx-2 rounded-[5px]">
                                  <p class="mb-2 tracking-tight m-2 py-2 pl-1 md:text-[12px]  text-[10px] h-20 overflow-auto text-[#A9A9A9]">{data?.link_info?.message}</p>
                                </div>
                              ...
                              <span className="flex justify-between gap-1  m-2">
                                <span className="flex justify-start gap-1 pt-3">
                                  {/* <span
                                    style={{ color: 'white' }}
                                    className=" bg-[#149E49] font-xs rounded-full text-xs px-2 h-5 pt-[2px] text-center cursor-pointer"
                                  >
                                    Active
                                  </span> */}
                                  ...
                                  <span
                                    ...
                                    className=" bg-blue-100 text-blue-800 outline-none font-xs rounded-lg text-xs px-2 h-5 pt-[2px] text-center "
                                  >
                                    Redirect
                                  </span>
                                ...
                                < span className="flex justify-end  ">
                                  ...
                                  <button
                                    ...
                                    className=" outline-none  font-xs text-red-500 rounded-full text-xs py-2 text-center "
                                  >
                                    {/* <FaTrash /> */}
